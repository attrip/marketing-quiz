(function(){
  ...
  const TEMPLATES = {
    ...
    cs: {
      title: 'CS改善ケース',
      guidelines: [
        'よくある困りごとに先回り（自己解決/導線）',
        '返答までの時間短縮と1回で解決',
        '同じ見た目の案内で迷わせない',
        '継続率/問い合わせ削減/工数削減で黒字筋'
      ],
      format: [
        '### 【問題】',
        '（現状の問い合わせ/解約の課題と条件）',
  ...
  const getTemplateKey = () => (document.getElementById('template')?.value || 'marketing');
  ...
  function buildMetaPrompt(opts){
    ...
    const tpl = TEMPLATES[getTemplateKey()] || TEMPLATES.marketing;
    ...
    return [
`あなたは、思考トレーニングを支援するアシスタントです。以下の条件とフレームワークに従って、ケーススタディ（問題と解答のセット）を1つ作成してください。`,
themeLine,
hintBlock,
"",
"絶対に守るべき条件:",
...
"注意: 出力には専門用語を含めないこと。数字の目安（黒字・回収の月数など）がある場合は自然に織り込むこと。"
  ...
  function buildQuickMeta(oneLine){
    ...
    const tpl = TEMPLATES[getTemplateKey()] || TEMPLATES.marketing;
    return [
`あなたは、思考トレーニングを支援するアシスタントです。次の一行をテーマとして使い、ケーススタディ（問題と解答のセット）を1つ作成してください。`,
`テーマ: 「${t}」`,
"",
"絶対に守るべき条件:",
"1) 平易な言葉だけを使い、専門用語は一切使わない。",
`   禁止例: ${banLine} など（日常語に言い換える）。`,
"2) 次の要点を反映する（理論名は出さない）:",
...tpl.guidelines.map(g=>`   - ${g}`),
"",
"アウトプットの形式（この通りに出力）:",
...tpl.format
  ...
  // Regenerate prompt when template changes
  document.getElementById('template')?.addEventListener('change', ()=>{
    generate();
    const line = $("quick_input").value;
    if(line) $("quick_meta").textContent = buildQuickMeta(line);
  });
